#!/usr/bin/env python3
"""DuplexCascade-PT bridge server (M3).
...
Bridges the browser demo to the three backends:
  ...
  * LLM: llama.cpp OpenAI-compatible server (http://127.0.0.1:8080/v1),
        serving the fine-tuned DuplexCascade-PT GGUF.
...
class BridgeServer:
    ...
    async def run(self, port: int) -> None:
        print(f"[DuplexCascade-PT] listening on 0.0.0.0:{port}", flush=True)
    ...
    async def llm_stream(self, history: list[dict], prime: str = "") -> list[str]:
        ...
        Returns text deltas; `prime` seeds the assistant turn start."""
        import httpx
...
def main() -> None:
    ...
    print(f"[DuplexCascade-PT] LLM backend: {args.llm_model} @ {args.llm_api_base}", flush=True)
