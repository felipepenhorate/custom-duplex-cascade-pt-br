#!/usr/bin/env bash
# M4 - launch the DuplexCascade-Distill stack:
#   1. llama-server   (llama.cpp)   - distilled GGUF  (OpenAI-compatible :8080)
...
#   GGUF=.../DuplexCascade-Distill-q4_k_m.gguf ./run_services.sh
...
# for details. Override with RUNS_ROOT to relocate.
RUNS_ROOT="${RUNS_ROOT:-/mnt/f/duplex_cascade_runs}"

# default GGUF: the distilled model; falls back to any gguf under the runs root.
# Override with GGUF=/path/to/model.gguf (e.g. to serve the SFT baseline).
GGUF="${GGUF:-$RUNS_ROOT/distill/full/export/gguf/DuplexCascade-Distill-q4_k_m.gguf}"
if [ ! -f "$GGUF" ]; then
  GGUF="$(ls "$RUNS_ROOT"/distill/full/export/gguf/*.gguf 2>/dev/null | head -1 || true)"
fi
...
# 1) llama.cpp server (OpenAI-compatible), serving the distilled model.
