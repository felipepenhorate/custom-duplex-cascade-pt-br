(() => {
  ...
  const updateUserText = (text) => {
    ...
    lastUserBubble = lastUserBubble || findLastBubbleByRole("user");
    const lastRole = getLastRole();
    if (lastRole === "assistant") {
      if (!backchannelHold) {
        userBaselineText = lastUserFullText;
        lastUserBubble = appendBubble("user");
      } else if (!lastUserBubble || lastUserBubble.wrap.parentNode !== chatList) {
        lastUserBubble = insertBubbleBeforeLast("user");
      }
    } else if (!lastRole) {
      lastUserBubble = appendBubble("user");
    ...
    const segment = getUserSegment(text);
    if (!segment && text === lastUserFullText) return;
    lastUserFullText = text;
    const msg = lastUserBubble;
    if (!msg) return;
    msg.meta.textContent = roleLabels.user;
    if (segment) {
      msg.body.textContent = segment;
    } else if (!text) {
  ...
  const appendAssistantText = (text) => {
    if (typeof text !== "string") return;
    if (text.length === 0) {
      if (lastAssistantBubble) lastAssistantBubble.body.textContent = "";
      return;
    }
    const lastRole = getLastRole();
    lastAssistantBubble = lastAssistantBubble || findLastBubbleByRole("assistant");
    if (!lastAssistantBubble) {
      if (lastRole === "user") {
        lastAssistantBubble = insertBubbleBeforeLast("assistant");
      } else {
        lastAssistantBubble = appendBubble("assistant");
      }
    } else if (lastRole === "user" && chatList && chatList.lastElementChild === lastAssistantBubble.wrap) {
      chatList.insertBefore(lastAssistantBubble.wrap, chatList.lastElementChild);
      updateActiveClass();
    }
    const msg = lastAssistantBubble;
    if (!msg) return;
    msg.meta.textContent = roleLabels.assistant;
    // Append text as text node to preserve existing HTML (like backchannel span)
    // If body has HTML children (like backchannel span), append text node after them
    // Otherwise, append text node to existing text content
    const textNode = document.createTextNode(text);
    msg.body.appendChild(textNode);
    // Ensure default text color (black) for new text - only affects direct text, not children
    if (msg.body.style.color) {
      msg.body.style.color = "";
    }
    scrollChat();
  };
  ...
  const connect = () => {
    ...
    ws.onclose = () => setWsBadge("Disconnected", false);
    ...
    ws.onmessage = async (evt) => {
      ...
      if (msg.type === "assistant_special") {
        if (typeof msg.text !== "string" || !msg.text.trim()) return;
        // dedup only backchannels; the user-finish token legitimately repeats
        // every turn and must always reset the turn state
        if (msg.text === USER_FINISH_TOKEN) {
          lastSpecial = null;
        } else if (msg.text !== BACKCHANNEL_TOKEN && msg.text === lastSpecial) {
          return;
        }
        lastSpecial = msg.text;
        const lastRole = getLastRole();
        if (msg.text === BACKCHANNEL_TOKEN) {
          backchannelHold = true;
          if (lastRole === "assistant") {
            lastAssistantBubble = lastAssistantBubble || findLastBubbleByRole("assistant");
            if (!lastAssistantBubble) {
              lastAssistantBubble = appendBubble("assistant");
            ...
            // Use a span element for backchannel text with light blue color
            lastAssistantBubble.body.innerHTML = "";
            const backchannelSpan = document.createElement("span");
            backchannelSpan.textContent = getBackchannelLabel(msg);
            backchannelSpan.style.color = "#60a5fa"; // Light blue color for backchannel
            lastAssistantBubble.body.appendChild(backchannelSpan);
            scrollChat();
          } else {
            appendAssistantBackchannel(getBackchannelLabel(msg));
          }
          return;
        }
        if (msg.text === USER_FINISH_TOKEN) {
