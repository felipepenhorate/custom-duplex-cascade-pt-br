...
fine-tuned to emit special turn-taking tokens (``\texttt{\textless|user is speaking|>}'',
``\texttt{\textless|user finish speaking|>}'', ``\texttt{\textless|user interruption|>}'', etc.) that
...
(e.g. ``\texttt{\textless|user finish speaking|>}'' before every system turn, probabilistic
...
``\texttt{\textless|user\_finish\_speaking|\textgreater}''$=10$, ``\texttt{\textless|user\_interruption|\textgreater}''$=5$.
...
\emph{Limitations.} This is work in progress: our evaluation is on a single backbone
(Qwen3-4B), a single language (pt-BR) and subset-scale benchmarks; the IFEval and GSM8K
numbers carry per-sample stderr of order $2$ pp. We did not ablate $\lambda$ against
math, nor test the full 50k-scale data regime; we plan to evaluate with substantially
more data in future work. The offline cache assumes the teacher is fixed,
...
intelligence. All experimental results and the associated code were reviewed
